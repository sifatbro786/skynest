...
export default function AdminError({ error, reset }) {
    ...
    return (
        <div className="panel-root flex min-h-dvh items-center justify-center p-6">
            <div className="pnl-card w-full max-w-2xl">
                <div className="pnl-card-head">
                    <div className="flex items-center gap-2">
                        ...
                        />
                        <h1 className="pnl-h2">পেজটি লোড করা যায়নি</h1>
                    </div>
                </div>
                ...
                <div className="pnl-card-body space-y-4">
                    ...
                    {isDev && error?.message ? (
                        <pre className="pnl-scroll max-h-72 whitespace-pre-wrap rounded-sm border border-[#d9b6aa] bg-[#f7ece8] p-3 text-xs leading-relaxed text-[#7d3220]">
                            {error.message}
                            {error.stack ? `\n\n${error.stack}` : ""}
                        </pre>
                    ) : null}
                    ...
                    {error?.digest ? (
                        <p className="text-xs text-ink-mute">
                            Error digest:{" "}
                            <code className="pnl-num rounded-sm bg-linen-deep px-1.5 py-0.5 font-mono">
                                {error.digest}
                            </code>
                        </p>
                    ) : null}
                    ...
                    <div className="flex flex-wrap gap-2 border-t border-line pt-3">
                        ...
                        <a href="/admin" className="pnl-btn pnl-btn-ghost">
                            ড্যাশবোর্ডে ফিরুন
                        </a>
                    </div>
                </div>
