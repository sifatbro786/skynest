...
/**
 * One SEO override row.
 *
 * `path` is the identity here, not `_id` — the admin panel keys its state by
 * route, and a row that does not exist yet is represented by the same shape
 * with empty values rather than by `null`. That is what lets the form treat
 * "never saved" and "saved blank" identically, which is also how the resolver
 * treats them.
 */
export function serializeSeoMeta(doc, path) {
  return {
    path: doc?.path ?? path ?? "",
    title: doc?.title ?? "",
    description: doc?.description ?? "",
